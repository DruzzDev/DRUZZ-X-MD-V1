...
async function handleAntideleteCommand(sock, chatId, message, isAdmin, botData, saveBotData, userId, args) {
    ...
    if (!match) {
        return sock.sendMessage(chatId, {
            text: `╭━━━〔 ${toBold("ᴀɴᴛɪ-ᴅᴇʟᴇᴛᴇ sᴇᴛᴜᴘ")} 〕━━━┈⊷\n` +
                   `┃ ☆ ${toBold("Status:")} ${config.enabled ? '✅ ᴇɴᴀʙʟᴇᴅ' : '❌ Disabled'}\n` +
                   `┃\n` +
                   `┃ ☆ ${toBold(".antidelete on")} - ᴇɴᴀʙʟᴇ\n` +
                   `┃ ☆ ${toBold(".antidelete off")} - ᴅɪsᴀʙʟᴇ\n` +
                   `╰━━━━━━━━━━━━━━━━━━┈⊷`
        }, {quoted: message});
    }
    ...
    if (match === 'on') {
        ...
        return sock.sendMessage(chatId, { text: '*ɪɴᴠᴀʟɪᴅ ᴄᴏᴍᴍᴀɴᴅ. ᴜsᴇ .ᴀɴᴛɪᴅᴇʟᴇᴛᴇ ᴛᴏ sᴇᴇ ᴜsᴀɢᴇ.*' }, {quoted:message});
    }

    saveAntideleteConfig(config);
    return sock.sendMessage(chatId, { text: `*ᴀɴᴛɪᴅᴇʟᴇᴛᴇ ${match === 'on' ? 'enabled' : 'disabled'}*` }, {quoted:message});
}
...
async function handleMessageRevocation(sock, revocationMessage) {
    try {
        ...
        let report = `╭━━━〔 ${toBold("ᴀɴᴛɪ-ᴅᴇʟᴇᴛᴇ ʀᴇᴘᴏʀᴛ")} 〕━━━┈⊷\n` +
                     `┃ 👤 ${toBold("sᴇɴᴅᴇʀ:")} @${senderName}\n` +
                     `┃ 🗑️ ${toBold("ᴅᴇʟᴇᴛᴇᴅ ʙʏ:")} @${deletedBy.split('@')[0]}\n` +
                     `┃ 🕒 ${toBold("ᴛɪᴍᴇ:")} ${new Date().toLocaleTimeString()}\n` +
                     `┃ 📂 ${toBold("ᴛʏᴘᴇ:")} ${original.mediaType || 'Text'}\n` +
                     `╰━━━━━━━━━━━━━━━━━━┈⊷\n\n`;

        if (original.content) {
            report += `*📝 ${toBold("ᴍᴇssᴀɢᴇ ᴄᴏɴᴛᴇɴᴛ:")}\n${original.content}*`;
        }
